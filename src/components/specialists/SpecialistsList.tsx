...
const SpecialistsList = () => {
  return <section className="section-padding bg-gradient-to-b from-white to-gray-50 py-[20px]">
      <div className="container-custom">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {specialists.map((specialist, index) => <Card key={specialist.id} className="overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100 bg-white/80 backdrop-blur-sm soft-shadow" style={{
          animationDelay: `${index * 0.1}s`
              ...
              <CardContent className="p-0">
                ...
                <div className="p-5 bg-white">
                  ...
                  <Button variant="outline" className="w-full border-primary/80 text-primary hover:bg-primary hover:text-white transition-all group">
                    <Calendar className="mr-2 h-4 w-4 group-hover:scale-110 transition-transform" />
                    <a href={specialist.superdocLink} target="_blank" rel="noopener noreferrer" className="w-full h-full">
                      Запазете час
                    </a>
                  </Button>
                </div>
